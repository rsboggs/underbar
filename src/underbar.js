(function() {
  ...
  _.last = function(array, n) {
    if (n === undefined) {
      ...
      var start = n > array.length - 1 ? 0 : array.length - n;
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(everyTrue, item) {
      if (!everyTrue) {
        return false;
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(item) {
      if (functionOrKey.constructor === Function) {
  ...
  _.sortBy = function(collection, iterator) {
    ...
    var compareFunction = function(a, b) {
      if (iterator.constructor === String) {
        var aValue = a[iterator], bValue = b[iterator];
      } else {
        var aValue = iterator(a), bValue = iterator(b);
      }
  ...
  _.zip = function() {
    ...
    // Find max length of arrays in arguments
    ...
    // Keep looping until zipped is same size as largest array
    for (var i = 0; i < maxLength; i++) {
      // Create temp array to hold each array's current index value
      var arr = [];
  ...
  _.flatten = function(nestedArray, result) {
    return _.reduce(nestedArray, function(flattened, next) {
      return flattened.concat(Array.isArray(next) ? _.flatten(next) : next);
    }, []);
  };
  ...
  _.intersection = function() {
    ...
    var uniqueList = _.uniq(_.flatten(args));
    var result = [];
    _.each(uniqueList, function(item) {
  ...
  _.difference = function(array) {
    ...
    var comparisonList = _.uniq(_.flatten(otherArrays));
    var result = [];
    _.each(array, function(item) {
      if (_.indexOf(comparisonList, item) === -1) {
        result.push(item);
      }
    });
    return result;
